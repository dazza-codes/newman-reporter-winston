...
class Newman {
  constructor(emitter, reporterOptions, options) {
    ...
    emitter.on("start", () => {
      this.logger.info(
        `Start collection run for collection "${newmanCollection.name}" (${newmanCollection.id})`
      );
      this.count = 1;
    });
    ...
    emitter.on("done", () => {
      this.logger.info(
        `Collection run completed for collection "${newmanCollection.name}" (${newmanCollection.id}):  ${this.count} tests executed`
      );
    });
  }
}
